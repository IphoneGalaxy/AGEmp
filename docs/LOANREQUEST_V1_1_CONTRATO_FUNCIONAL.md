...
Este documento **não substitui** o congelamento da v1; define somente **extensões** permitidas quando a implementação for autorizada pela governança do repositório (contrato atualizado + QA + alterações em [`firestore.rules`](../firestore.rules) e cliente).
...
Esta seção **prepara** revisão técnica; o PR deve espelhar 1:1 em [`firestore.rules`](../firestore.rules) **e** em helpers JS.
...
## Critérios de entrada para iniciar código (checklist rápido)

- [ ] Governança aceita este arquivo como especificação vinculável.
- [ ] Escolha A vs B sobre `updatedAt` em atualizações de leitura.
- [ ] Matriz QA v1.1 criada e promovível a checklist de smoke mínimo: [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md)
- [ ] Atualização prevista das rules + índices (se aplicável apenas em consultas/listas já existentes) descrita antes de merge.
