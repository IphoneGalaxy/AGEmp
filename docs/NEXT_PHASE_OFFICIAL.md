...
- **Próximo recorte recomendado (pós-Bloco 2):** mini ADR **«Identidade pública e snapshots de nomes em vínculos/pedidos»** — documento vivo [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) (**apenas planejamento nesta data**; financeiro segue **local-first**, sem sync remoto financeiro / sem contrato remoto / sem `payment.linkContext` / sem `calculations.js` nesta linha); depois **«Visão Fornecedores»**. Roadmap [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md). **Bloco 2** não alterou o facto de **`loanRequests`** ser só pré-financeiro na nuvem; a conversão é **100% local** após confirmação humana (**o app não transfere dinheiro**).
...
**Próxima fase recomendada:** mini ADR [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) (**publicado** como direção; implementação ordenada §13); **depois** **«Visão Fornecedores / UX de relacionamento por papel»** — ver [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md) e [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) § Limitações.
...
| 2026-05-05 | Mini ADR [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) — direção snapshots (`links`/`loanRequests`); só docs nesta rodada. |
