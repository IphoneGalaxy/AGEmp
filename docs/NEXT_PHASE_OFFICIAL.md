...
**Próximo recorte de produto** não está fixado neste arquivo — candidatos em backlog ([`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16.6; [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md)): revogação remota de vínculo; **A2b/A2c**; refinamentos registry (backup/export) e UI. **Linha ortogonal (`loanRequests`):** [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md) — **`clientDebtLedger`** (**dados locais neste aparelho**, **sem sincronização financeira remota**); Subfases **A–D3** **concluídas** (**`5fc8a58`** … **`eedbd2e`**, smoke **D3** OK **2026-05-17** — ADR §20); **pacote «Pré-Sync Local Hardening»** (**UX/lembretes/export JSON específico**) **fechado documentalmente** (**2026-05-16** — [`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md); commits **`d6f69af`**, **`8228b5c`**, **`b86ae9b`**); Firebase **não** é fonte financeira autoritativa do ledger; **`pedido aprovado` não cria dívida local automaticamente**; **`calculations.js`**, **`firestore.rules`**, Firebase SDK e **`payment.linkContext`** intocados nesta linha — ver ADR §13–§21 e backlog §18.
...
| 2026-05-16 | **Pacote «Pré-Sync Local Hardening» — fecho documental (Onda 2B):** plano [`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md); commits **`d6f69af`**, **`8228b5c`**, **`b86ae9b`**; smoke final OK — ADR §21 / matriz QA; **sem sync financeiro remoto**. |
