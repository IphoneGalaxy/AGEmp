...
**Roadmap evolutivo `loanRequest` (A1–F, planejamento vivo — não equivale ao histórico em `plans/completed/`):** [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md). **Bloco 2 (conversão governada)** — **funcionalmente fechado** (2026-05-04): ADR [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md); commits **`624c725`**, **`3badcbc`**, **`5dd4c36`**; QA/smoke em [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md) § Bloco 2. **Mini ADR «snapshots de nomes»:** [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) — **funcionalmente fechada** (**`6793461`** … **`28f3f4a`**; deploy **`agemp-financas-pro`** pós-**`cdc55d9`**; §17). **«Visão Fornecedores + Governança local»:** **fechada** — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md). **«Minhas dívidas» / financeiro local do cliente:** [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md) — **`clientDebtLedger`** **funcionalmente fechado nesta etapa** (**Subfases A–D3**, **`5fc8a58`** … **`eedbd2e`**); ledger **separado** de **`clients[]`** e de **`loanRequestConversionRegistry`**; backup/import/auto-backup **incluem** o ledger (**dados locais neste aparelho**); **sem sincronização financeira remota** com fornecedor; smoke manual **D3** **OK integral** (**2026-05-17** — ADR §20); **`calculations.js`**, **`firestore.rules`**, Firebase SDK e **`payment.linkContext`** **intocados** nesta linha; **pacote «Pré-Sync Local Hardening»** ([`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md)) — **fechado documentalmente** (**Onda 2B**, **2026-05-16**; commits **`d6f69af`**, **`8228b5c`**, **`b86ae9b`**); **Subfase E**, importação do JSON dedicado, CSV e refinamentos maiores — backlog com ADR própria.
...
|--------|----------------|------------|
| Domínio financeiro local | clientes, contratos, pagamentos, caixa, dashboard, backups (inclui **`clientDebtLedger`** nos dados exportados/restaurados neste aparelho), preferências operacionais por escopo | `localStorage` particionado (`anonymous` · `account:{uid}`) |
...
- Backup / import / auto-backup (inclui **`clientDebtLedger`** no pacote JSON, com compatibilidade a backups **sem** esse campo — ledger efetivo vazio normalizado).
...
| Dimensão | Situação |
|----------|----------|
| **Concluído (alto nível)** | Base remota identidade/vínculo; escopo local + legado; clareza de contexto; `linkContext` v1 em cliente, contrato e UI de pagamentos (derivada); visão operacional derivada por vínculo (`linkOperationalDerive` + refinamento em `ClientsList`); **bloco `ClientView`** com resumo operacional, espelho explícito em pagamentos e estados vazio/erro/divergência; **pacotes `loanRequest` v1 e v1.1 (RB+CN pré-financeiro)** **fechados** com smoke manual OK — [`QA_MATRIX_LOANREQUEST_V1.md`](./plans/completed/QA_MATRIX_LOANREQUEST_V1.md) · [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md); **Bloco 2 — conversão governada** **fechado** (**`624c725`**, **`3badcbc`**, **`5dd4c36`** — [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md)); **«Visão Fornecedores + Governança de vínculo/local»** — Pacotes **1–3** **fechados** (**`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`** — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16); **«Minhas dívidas» (`clientDebtLedger`)** — **funcionalmente fechado nesta etapa** (**`5fc8a58`** … **`eedbd2e`**, smoke **D3** OK **2026-05-17**); **pacote «Pré-Sync Local Hardening»** documentalmente **fechado** (**2026-05-16** — [`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md)); **dados locais neste aparelho**, **sem sincronização financeira remota** do ledger; ADR §20 / QA § smoke D3 + § Pacote Pré-Sync. |
| **Validado** | Automático recorrente (`vitest`/build); QA manual [`QA_MATRIX_GENERAL.md`](./QA_MATRIX_GENERAL.md); fatia vínculo [`QA_MATRIX_LINK_OPERATIONAL_VIEW.md`](./QA_MATRIX_LINK_OPERATIONAL_VIEW.md); **`loanRequest` v1 e v1.1** e **Bloco 2** com smoke/conferência conforme matrizes dedicadas e ADR; § **«Visão Fornecedores + Governança local»** em [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md); smoke manual **«Minhas dívidas» D3** OK integral (**2026-05-17**); smoke **Pacote Pré-Sync Local Hardening** OK (**2026-05-16**); ADR pagamentos [`ADR_PAYMENT_LINK_CONTEXT.md`](./ADR_PAYMENT_LINK_CONTEXT.md). |
...
| **Próximo foco** | **«Visão Fornecedores + Governança local»:** **fechada**. **Mini ADR snapshots:** **fechada**. **«Minhas dívidas»:** linha base **fechada**; **pacote «Pré-Sync Local Hardening»** (**UX fronteira, lembretes derivados, export JSON específico**) **fechado** ([`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md)) — evoluções seguintes só backlog (**Subfase E**, quota **`localStorage`**, **import REST do JSON dedicado**, CSV, lembretes mais avançados, alinhamento ledger ↔ **`calculations.js`**) **com ADR própria**; **sem** integração Painel global nem sync financeiro remoto sem decisão explícita. **Backlog (priorização em sprint própria):** revogação/desfazer vínculo remoto; **A2b/A2c**; refinamento fallback **`links`** legados sem **`supplierDisplayNameSnapshot`**; marcação remota «converted»; backup/export do registry local; **fora de escopo:** sync financeiro remoto; `payment.linkContext` sem ADR; motor por vínculo sem plano. |
...
| 2026-05-03 | **Roadmap evolutivo vivo `loanRequest`:** [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md) registra **A1–F** como planejamento complementar (`A1` sinalização global sem rules; **`A2` arquivamento** com campos exemplo `archivedByClient`/`archivedBySupplier` só quando houver decisão/implementação; **E/F** sob ADR); [`plans/completed/`](./plans/completed/) continua **somente histórico**. |
...
| 2026-05-04 | **Bloco2-0:** [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) criado (posteriormente **aprovado** — ver linha seguinte). |
| 2026-05-04 | **Bloco 2 fechado + Bloco2-E:** conversão governada documentada (commits **`624c725`**, **`3badcbc`**, **`5dd4c36`**); smoke utilizador OK; matriz QA v1.1 § Bloco 2; ADR/handoff/NEXT/roadmap/FIRESTORE/plans README; **próximo:** snapshots de nomes · Visão Fornecedores. |
| 2026-05-04 | **Governança Bloco 2 (aprovação inicial):** ADR **aprovado**; **D6** fechada; MVP sem marcação remota; **A2b/A2c** backlog — **implementação A–D** registada nos commits **`624c725`**, **`3badcbc`**, **`5dd4c36`**; **Bloco2-E** doc na mesma data. |
| 2026-05-04 | **Fase A1 do Bloco 1 concluída:** **`dcc9f80`** · **`4951bdf`**; matriz QA v1.1 e handoff alinhados; **próxima subfase** **A2a** (planejamento, sem código). |
...
| 2026-05-05 | Mini ADR snapshots — **fecho funcional + Subfase 7 (QA/docs):** commits **`6793461`**–**`28f3f4a`**; deploy **`agemp-financas-pro`**; smoke/registo em [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md); próximo **Visão Fornecedores**. |
| 2026-05-05 | Mini ADR [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) — documentação oficial snapshots (`links`/`loanRequests`); sem código/rules/testes na rodada inicial (Subfase 0). |
| 2026-05-16 | **«Minhas dívidas» / `clientDebtLedger`:** Subfases **A–D1** em código (**`5fc8a58`**, **`e24eb25`**, **`0f2c43b`**, **`40fa3a4`**); **D2** atualiza docs vivos (**`eedbd2e`** — HANDOFF/checkpoint/NEXT/roadmap/matriz/plans README/ADR); backup/import/auto-backup passam a carregar **`clientDebtLedger`** nos **dados locais neste aparelho**; **`calculations.js`**, **`firestore.rules`**, SDK e **`payment.linkContext`** intocados. |
| 2026-05-17 | **«Minhas dívidas» — D3 documental + smoke manual OK integral:** linha **funcionalmente fechada nesta etapa** (ADR §20); **`calculations.js`**, **`firestore.rules`**, SDK Firebase, **`payment.linkContext`**, sync financeiro remoto e contrato remoto autoritativo **intocados** nesta linha. |
| 2026-05-16 | **Pacote «Pré-Sync Local Hardening» — Onda 2B:** docs vivos + plano movido para [`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md); commits **`d6f69af`**, **`8228b5c`**, **`b86ae9b`**; smoke final OK (matriz QA § Pacote Pré-Sync); **sem sync financeiro remoto**. |
