...
**Base estável de referência (LKG):** `lkg-2026-05-01-loanrequest-v1-complete` — commit exato: `git rev-parse lkg-2026-05-01-loanrequest-v1-complete` (inclui registro formal do pacote **`loanRequest` v1** após smoke manual OK). **`loanRequest` v1.1 — Fatia RB (read markers):** LKG **`lkg-2026-05-03-loanrequest-v1-1-rb`** (implementação **`7270409`**; apenas RB; pacote nominal v1.1 **completo** ainda pendente da fatia **CN**). Marco histórico do bloco **ClientView:** `lkg-2026-04-30-clientview-operational-link-block-complete`. Cadeia anterior relevante: `lkg-2026-04-28-link-operational-view` · `28f7936`; pacotes intermediários `lkg-2026-04-29-clientview-operational-link-reading`, `lkg-2026-04-30-clientview-payment-derived-reading`.
...
| Dimensão | Situação |
|----------|----------|
| **Concluído (alto nível)** | Base remota identidade/vínculo; escopo local + legado; clareza de contexto; `linkContext` v1 em cliente, contrato e UI de pagamentos (derivada); visão operacional derivada por vínculo (`linkOperationalDerive` + refinamento em `ClientsList`); **bloco `ClientView`** com resumo operacional, espelho explícito em pagamentos e estados vazio/erro/divergência; **pacote `loanRequest` v1** (pré-financeiro remoto + UI Conta) **fechado** com smoke manual OK — [`QA_MATRIX_LOANREQUEST_V1.md`](./plans/completed/QA_MATRIX_LOANREQUEST_V1.md); testes e LKGs na linha. |
| **Validado** | Automático recorrente (`vitest`/build); QA manual [`QA_MATRIX_GENERAL.md`](./QA_MATRIX_GENERAL.md); fatia vínculo [`QA_MATRIX_LINK_OPERATIONAL_VIEW.md`](./QA_MATRIX_LINK_OPERATIONAL_VIEW.md); fatia **`loanRequest` v1** [`QA_MATRIX_LOANREQUEST_V1.md`](./plans/completed/QA_MATRIX_LOANREQUEST_V1.md) (smoke real **OK integral**, sem NOK crítico); ADR atual [`ADR_PAYMENT_LINK_CONTEXT.md`](./ADR_PAYMENT_LINK_CONTEXT.md). |
...
| **Próximo foco** | Ciclo local-first **praticamente encerrado** no gate geral (**F2**/**F5** OK conforme [`QA_MATRIX_GENERAL.md`](./QA_MATRIX_GENERAL.md)); **pacote `loanRequest` v1 fechado**; **`loanRequest` v1.1 — Fatia RB** registrada (**`7270409`**, deploy rules **`agemp-financas-pro`**, smoke manual OK — LKG **`lkg-2026-05-03-loanrequest-v1-1-rb`**, cf. [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md)); **fatia CN** (`counteroffer` / contraproposta) **ainda não iniciada**; direção atual em [`NEXT_PHASE_OFFICIAL.md`](./NEXT_PHASE_OFFICIAL.md); contexto histórico da execução v1 em [`plans/completed/PLANEJAMENTO_MESTRE_LOANREQUEST_PRE_FINANCEIRO.md`](./plans/completed/PLANEJAMENTO_MESTRE_LOANREQUEST_PRE_FINANCEIRO.md). |
...
| 2026-05-03 | **Planejamento `loanRequest` v1.1** documentado (`readByClientAt`/`readBySupplierAt` primeiro, contraproposta depois na ordem de execução; pacote nominal v1.1) — [`LOANREQUEST_V1_1_CONTRATO_FUNCIONAL.md`](./LOANREQUEST_V1_1_CONTRATO_FUNCIONAL.md) · [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md). |
| 2026-05-03 | **Fatia RB `loanRequest` v1.1** implementada (`readByClientAt`, `readBySupplierAt`; commit **`7270409`**); **`firestore.rules`** publicadas com sucesso no projeto Firebase **`agemp-financas-pro`**; smoke manual humano OK, sem NOK crítico informado; LKG **`lkg-2026-05-03-loanrequest-v1-1-rb`**. Subfase natural seguinte (**não iniciada**): CN / `counteroffer` conforme especificação v1.1. |
