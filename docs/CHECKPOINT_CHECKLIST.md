...
| **Próximo foco** | **«Visão Fornecedores + Governança local»:** **fechada** — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16 (**`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`**). **Mini ADR snapshots:** **fechada** ([`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) §17 — commits **`6793461`** … **`28f3f4a`**). **Backlog (priorização em sprint própria):** revogação/desfazer vínculo remoto; **A2b/A2c** (arquivamento **remoto** `loanRequests` — **não** confundir com arquivamento **local** da fase Visão Fornecedores); refinamento fallback **`links`** legados sem **`supplierDisplayNameSnapshot`**; marcação remota «converted»; backup/export do registry local; refino visual/componentização. **Fora de escopo até decisão explícita:** sync financeiro remoto; `payment.linkContext` sem ADR; motor por vínculo sem plano. |
