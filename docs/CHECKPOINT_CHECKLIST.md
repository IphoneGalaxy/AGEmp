...
| **Próximo foco** | Ciclo local-first **praticamente encerrado** no gate geral (**F2**/**F5** OK conforme [`QA_MATRIX_GENERAL.md`](./QA_MATRIX_GENERAL.md)); **`loanRequest` v1+v1.1 (RB+CN) encerrados** — LKG **`lkg-2026-05-03-loanrequest-v1-1`**. **Plano executável ativo — Bloco 1 (`loanRequests`):** [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) — **A1**, **A2a (documental)**, **B1 (análise)** concluídas; **A2b/A2c** e **B2** **não** implementadas. **Próxima subfase recomendada:** **`B2`** (alerta fornecedor com **`availableMoney`**, `requestedAmount/100`) **salvo** decisão explícita de **A2b** antes. Roadmap: [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md). **Fora de escopo até decisão explícita:** conversão automática pedido→contrato; sync financeiro remoto; persistência `payment.linkContext` sem ADR. Contexto histórico da execução v1 em [`plans/completed/PLANEJAMENTO_MESTRE_LOANREQUEST_PRE_FINANCEIRO.md`](./plans/completed/PLANEJAMENTO_MESTRE_LOANREQUEST_PRE_FINANCEIRO.md). |
