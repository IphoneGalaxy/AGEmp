...
| **Próximo foco** | **«Visão Fornecedores + Governança local»:** **fechada**. **Mini ADR snapshots:** **fechada**. **«Minhas dívidas»:** **fechada nesta etapa** — evoluções só backlog (**Subfase E**, UX, lembretes locais avançados, quota **`localStorage`**, relatório/export específico, alinhamento futuro ledger ↔ **`calculations.js`**) **com ADR própria**; **sem** integração Painel global nem sync financeiro remoto sem decisão explícita. **Backlog (priorização em sprint própria):** revogação/desfazer vínculo remoto; **A2b/A2c**; refinamento fallback **`links`** legados sem **`supplierDisplayNameSnapshot`**; marcação remota «converted»; backup/export do registry local; refinamento visual «Minhas dívidas»/detalhe fornecedor; **fora de escopo:** sync financeiro remoto; `payment.linkContext` sem ADR; motor por vínculo sem plano. |
