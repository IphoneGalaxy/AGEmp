...
| **Próximo foco** | **«Visão Fornecedores + Governança local»:** **fechada** — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16 (**`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`**). **Mini ADR snapshots:** **fechada** ([`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) §17 — commits **`6793461`** … **`28f3f4a`**). **«Minhas dívidas»:** Subfases **A–D1** entregues; **D2** documental; **D3**/fechamento ou restante Subfase D conforme [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md) — **sem** integração ao Painel global (**Subfase E**) sem ADR. **Backlog (priorização em sprint própria):** revogação/desfazer vínculo remoto; **A2b/A2c** (arquivamento **remoto** `loanRequests` — **não** confundir com arquivamento **local** da fase Visão Fornecedores); refinamento fallback **`links`** legados sem **`supplierDisplayNameSnapshot`**; marcação remota «converted»; backup/export do registry local; refino visual «Minhas dívidas»/detalhe fornecedor; **fora de escopo:** sync financeiro remoto; `payment.linkContext` sem ADR; motor por vínculo sem plano. |
