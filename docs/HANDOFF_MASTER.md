...
- **Bloco 1 (`loanRequests` — operacional):** **funcionalmente fechado** (governança **Opção A**, 2026-05-04). Plano **arquivado** (histórico): [`plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md). **Entregue:** **A1a** (`dcc9f80`, `countUnreadLoanRequests` + testes) · **A1b** (`4951bdf`, badges na Conta) · **A2a** (decisões arquivamento por lado, documental — **`8b3cadb`**) · **B1** (métrica **`availableMoney`** / **`calculateGlobalStats`**) · **B2** (`07ef7e5`, alerta não bloqueante no fornecedor). **A2b/A2c** **não** implementadas — **backlog / futuro** (organização por arquivamento); **não** impedem o fechamento funcional. **Mini ADR «Identidade pública e snapshots de nomes»:** **funcionalmente fechada** — [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) §17; commits **`6793461`** · **`eb48083`** · **`cdc55d9`** · **`9d2c4ad`** · **`9f4351f`** · **`bb03633`** · **`28f3f4a`**; deploy **`firestore:rules`** em **`agemp-financas-pro`** após **`cdc55d9`**. Guardrails: sem sync financeiro remoto/contrato remoto/`payment.linkContext`/mudanças em **`calculations.js`** nesta linha. **«Visão Fornecedores + Governança de vínculo/local»:** **funcionalmente fechada** (Pacotes 1–3) — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16; commits **`0be3e0b`** · **`c921d8d`** · **`a6c2d8c`**. **Sem** alteração a **`firestore.rules`** nesta fase; **sem** revogação remota de vínculo. **Backlog:** revogação/desfazer vínculo remoto; **A2b/A2c** (arquivamento **remoto** por lado — distinto do arquivamento **local** da fase); refinamento backup/export do registry; melhorias visuais/componentização se necessário.
- **Bloco 2 (`loanRequests` → contrato local — conversão governada):** **funcionalmente fechado** (2026-05-04). [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md). **Commits:** **`624c725`** · **`3badcbc`** · **`5dd4c36`**. Entrega: fornecedor regista contrato **local** após revisão + confirmação humana; **`convertedFromLoanRequestId`** e opcional **`loan.linkContext`**; **sem** Firestore/rules/`calculations.js`/`payment.linkContext`/sync financeiro remoto/marcação remota converted; microcopy **não transfere dinheiro**; **Bloco2-E** — QA/smoke + docs vivos.
...
- **Mini ADR snapshots de nomes:** **fechada** — [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) §13–§17; commits **`6793461`** … **`28f3f4a`**; deploy rules **`agemp-financas-pro`** pós-**`cdc55d9`**. **«Visão Fornecedores + Governança de vínculo/local»:** **fechada** — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16 · **`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`**. **Bloco 2 fechado:** [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) · **`624c725`**, **`3badcbc`**, **`5dd4c36`** · QA [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md) § Bloco 2 + § mini ADR + § «Visão Fornecedores».

**Factual (codigo existente):** **`loanRequests`** **v1+v1.1** permanece **somente pré-financeira**; inclui opcionalmente **`clientDisplayNameSnapshot`** / **`supplierDisplayNameSnapshot`** na criação (imutáveis). **`links`** inclui os mesmos snapshots (escrita por papel). **Bloco 2** converte só para contrato **local** (**`624c725`** … **`5dd4c36`**, uso do snapshot cliente em **`28f3f4a`**) — Firebase **não** é fonte financeira autoritativa; **sem** sync financeiro remoto / contrato remoto / **`payment.linkContext`** / mudanças em **`calculations.js`** nesta linha. **«Visão Fornecedores + Governança local»** entregue (**`c921d8d`**, **`a6c2d8c`**) — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16; próximos recortes em backlog no mesmo ADR e em [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md).

**ADR — Financeiro Local do Cliente / «Minhas dívidas»:** [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md) — modelo **`clientDebtLedger`** separado de **`clients[]`** e de **`loanRequestConversionRegistry`**; **dados locais neste aparelho** (por escopo); incluído em backup/export/import/auto-backup (**`40fa3a4`**); Firebase apenas vínculo/pedido **pré-financeiro** — **sem sincronização financeira remota** do ledger nem contrato financeiro remoto autoritativo; **`pedido aprovado` não cria dívida local automaticamente**; **`calculations.js`**, **`firestore.rules`**, Firebase SDK e **`payment.linkContext`** **intocados** nesta linha; **funcionalmente fechada nesta etapa:** **`5fc8a58`**, **`e24eb25`**, **`0f2c43b`**, **`40fa3a4`**, **`eedbd2e`**, smoke **D3** OK (**2026-05-17** — ADR §20); **pacote «Pré-Sync Local Hardening»** (UX fronteira plataforma/local, lembretes derivados, export JSON específico do ledger) **encerrado documentalmente** (**Onda 2B**, **2026-05-16**) — plano histórico [`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md); commits **`d6f69af`** · **`8228b5c`** · **`b86ae9b`**; **Subfase E** (Painel global, alinhamento **`calculations.js`**, import do JSON dedicado, CSV, etc.) — backlog com ADR própria quando aplicável.
...
- até **`loanRequests`** **v1+v1.1** (solicitações pré-financeiras na Conta, marcadores `readBy*`, fatia CN com `counteroffer` / `counteroffer_declined`; snapshots opcionais **`clientDisplayNameSnapshot`** / **`supplierDisplayNameSnapshot`** — mini ADR **`6793461`** … **`28f3f4a`**; **Bloco 1 / A1:** badges agregados de novidade nos botões da Conta — commits **`dcc9f80`**, **`4951bdf`**; **Bloco 1 / B2:** alerta de disponível local no painel fornecedor — **`07ef7e5`**; ver [`FIRESTORE_LOANREQUESTS.md`](./FIRESTORE_LOANREQUESTS.md) e [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md)); **Bloco 2:** conversão governada **`approved` → contrato local** — **`624c725`**, **`3badcbc`**, **`5dd4c36`**, nome amigável do pedido na conversão **`28f3f4a`** — [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md); **«Visão Fornecedores + Governança local»** (cliente/registry/arquivamento local) — **`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`** — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16 (**sem** mudança Firestore/rules nesta fase);
...
**Bloco 1 — Fase A1 (2026-05-04, concluída):** sinalização agregada na **Conta** — **`dcc9f80`** `countUnreadLoanRequests` + testes · **`4951bdf`** badges nos botões **“Abrir solicitações”** / **“Abrir pedidos recebidos”** em `AccountScreen.jsx` (somente `count > 0`, por papel; leitura sob demanda; **sem** listener global). **Não** alterou `firestore.rules`, schema, `calculations.js`, `App.jsx`, `Settings.jsx`, `payment.linkContext`, sync financeiro nem contrato automático.

**Bloco 1 — fechamento funcional (Opção A — 2026-05-04):** **A1a, A1b, A2a, B1, B2** entregues; **A2b/A2c** em **backlog** (implementação futura de arquivamento por lado — **não** concluídas). Registros históricos e subfases: [`plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md).

**Bloco 1 — Subfase A2a (decisões de arquivamento, documental — concluída):** contrato **por lado** (`archivedByClient` / `archivedBySupplier` **planejados**, **não** no Firestore ainda); **só** status terminais; desarquivar **permitido**; **`updatedAt` intocado** no arquivo/desarquivo; exclusão/`delete` **fora**; rules/UI **só** nas futuras **A2b**/**A2c** — detalhe em [`plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) §6.

**Bloco 1 — Subfase B1 (métrica para alerta B2 — análise concluída):** confirmado uso de **`availableMoney`** (retorno de **`calculateGlobalStats`** em `calculations.js`), alinhado ao **“Total Disponível”** do Painel. **B2** compara **`requestedAmount / 100`** (reais) com **`availableMoney`**; alerta **informativo**, **não bloqueante**, **referência local** (não saldo bancário); **sem** alterar o motor; **sem** escrita Firestore motivada pelo alerta. Ver [`plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) §6 B1.

**Bloco 1 — Subfase B2 (alerta fornecedor — concluída):** **`07ef7e5`** — repasse **`availableMoney`** desde **`globalStats`**: **`App.jsx`**, **`Settings.jsx`**, **`AccountScreen.jsx`**, **`LoanRequestsSupplierPanel.jsx`**. Alerta só em **`pending`** / **`under_review`** quando **`requestedAmount / 100 > availableMoney`**; **sem** terminais nem **`counteroffer`** no escopo validado; aprovação **não** bloqueada; **`calculations.js`**, **`firestore.rules`**, schema, transições, **`payment.linkContext`**, sync remoto e contrato automático **intocados**. Ver plano arquivado §6 B2 e [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md) (Fase B).

**Mini ADR snapshots de nomes — fechada:** [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) §13–§17; commits **`6793461`** … **`28f3f4a`**; QA/smoke § mini ADR em [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md). **«Visão Fornecedores + Governança local» — fechada:** [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16 · **`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`**. **Referência Bloco 2 (fechado):** [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) · **`624c725`** · **`3badcbc`** · **`5dd4c36`** (+ uso snapshot cliente na conversão **`28f3f4a`**) · [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md) § Bloco 2. **A2b/A2c** permanecem **backlog** opcional (arquivamento **remoto** — distinto do arquivamento **local** da fase Visão Fornecedores). [`plans/completed/`](./plans/completed/) **só** histórico (ver [`plans/README.md`](./plans/README.md)).
...
| 2026-05-04 | **Plano executável Bloco 1** (`loanRequests` — A1, A2a, B): plano em `docs/`; §2 e §9 alinhados. |
| 2026-05-04 | **Governança Opção A:** **Bloco 1 funcionalmente fechado**; plano movido para [`plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md); **A2b/A2c** em backlog; **Bloco 2** próxima fase recomendada. |
...
| 2026-05-04 | **Subfase A2a (arquivamento — decisões) documentada:** §2/§9; **sem** código/rules. **Próxima recomendada:** **B1** (salvo priorizar A2b). |
| 2026-05-04 | **Subfase B1 (métrica disponível) documentada:** `availableMoney` / `calculateGlobalStats`; **B2** pode seguir. **Próxima:** **B2** (salvo A2b). |
| 2026-05-04 | **Subfase B2 concluída:** **`07ef7e5`** — alerta não bloqueante no painel fornecedor; roadmap com **Bloco 2** futuro. **Seguido de:** **Opção A** — fechamento funcional Bloco 1 e arquivo do plano em **`plans/completed/`**. |
| 2026-05-04 | **ADR — Financeiro Local do Cliente / «Minhas dívidas»:** [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md) — **`clientDebtLedger`** planejado local-first; **`calculations.js`** intocado na fase documentada inicial. |
| 2026-05-04 | **Bloco2-0:** ADR/plano vivo **Bloco 2** em [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) criado (**posteriormente aprovado** — ver linha seguinte). |
| 2026-05-04 | **Bloco 2 funcionalmente fechado:** conversão governada entregue (**`624c725`**, **`3badcbc`**, **`5dd4c36`**); smoke manual OK; ADR/QA/checkpoint/roadmap/NEXT/plans README alinhados (**Bloco2-E** doc); **sem** mudança remota/`rules`/`calculations.js` pelo MVP; **próximo:** mini ADR snapshots de nomes · depois Visão Fornecedores. |
| 2026-05-05 | **Mini ADR «Identidade pública e snapshots de nomes»** publicado em [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) — só documentação inicial (Subfase 0); sem `src/`, rules nem testes naquela rodada. |
| 2026-05-05 | **Mini ADR — fecho funcional + Subfase 7 (QA/docs):** implementação **`6793461`** … **`28f3f4a`**; deploy rules **`agemp-financas-pro`** pós-**`cdc55d9`**; smoke Mello/Guilherme + segunda conversão bloqueada + permission denied corrigido após deploy; limitações/backlog em ADR §17 e [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md); próximo foco **Visão Fornecedores**. |
| 2026-05-05 | **«Visão Fornecedores + Governança de vínculo/local» — Pacotes 1–3 fechados:** ADR §16 [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md); commits **`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`**; **`firestore.rules`** / modelo `loanRequests` **intocados**; QA/smoke § dedicado em [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md); backlog §16.6 no ADR (revogação remota; **A2b/A2c**; registry backup/UI). |
| 2026-05-17 | **«Minhas dívidas» — Subfase D3 documental + smoke manual OK:** linha **funcionalmente fechada nesta etapa**; commits de referência **`5fc8a58`** … **`eedbd2e`**; **sem NOK crítico** no smoke humano (ADR §20); **`calculations.js`**, **`firestore.rules`**, SDK Firebase, **`payment.linkContext`** e sync financeiro remoto **intocados**; próximo só backlog (**Subfase E**/UX/refinos) com ADR própria. |
| 2026-05-16 | **Pacote «Pré-Sync Local Hardening» — Onda 2B documental + smoke final OK:** UX/lembretes (`d6f69af`), export JSON específico (`8228b5c`), correção `debtStatusLabelPt` (`b86ae9b`); plano arquivado em [`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md); **sem sync financeiro remoto**; ADR/handoff/checkpoint/matriz atualizados. |
