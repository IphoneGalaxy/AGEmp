...
- **Bloco 1 (`loanRequests` — operacional):** **funcionalmente fechado** (governança **Opção A**, 2026-05-04). Plano **arquivado** (histórico): [`plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md). **Entregue:** **A1a** (`dcc9f80`, `countUnreadLoanRequests` + testes) · **A1b** (`4951bdf`, badges na Conta) · **A2a** (decisões arquivamento por lado, documental — **`8b3cadb`**) · **B1** (métrica **`availableMoney`** / **`calculateGlobalStats`**) · **B2** (`07ef7e5`, alerta não bloqueante no fornecedor). **A2b/A2c** **não** implementadas — **backlog / futuro** (organização por arquivamento); **não** impedem o fechamento funcional. **Próxima fase de produto (pós-Bloco 2):** mini ADR **«Identidade pública e snapshots de nomes»** — fonte viva [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) (**documentação**; implementação **não** iniciada nesta rodada); depois **«Visão Fornecedores»** — ver também [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) § Limitações.
...
- **Próxima fase recomendada (produto, pós-Bloco 2):** mini ADR **publicado** [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) — direção **perfil + snapshots em `links` + snapshots em `loanRequests`** (`clientDisplayNameSnapshot`, `supplierDisplayNameSnapshot`; MVP histórico; herança pedido ← link). **Implementação:** por subfases no próprio ADR (§13); **sem** código nem rules nesta rodada documental. **Depois:** *«Visão Fornecedores / UX de relacionamento por papel»*. **Bloco 2 fechado:** [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) · commits **`624c725`**, **`3badcbc`**, **`5dd4c36`** · QA/smoke em [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md) § Bloco 2.

**Factual (codigo existente):** a colecao **`loanRequests`** cobre **v1** e **v1.1** (marcadores de leitura, contraproposta, terminais CN) e permanece **somente pre-financeira**; **Bloco 2** entrega **conversão governada** para contrato **local** nos commits **`624c725`**, **`3badcbc`**, **`5dd4c36`** — **sem** tornar Firebase fonte financeira autoritativa. Campos remotos de snapshot **ainda não existem** até execução das subfases do [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md). **Seguinte produto planejado:** implementação dessa mini ADR · Visão Fornecedores — [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md).
...
**Próximo recorte recomendado:** mini ADR [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) (**planejamento aprovado**; execução futura §13); em seguida **«Visão Fornecedores»** (cliente). **Referência Bloco 2 (fechado):** [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) · **`624c725`** · **`3badcbc`** · **`5dd4c36`** · [`QA_MATRIX_LOANREQUEST_V1_1.md`](./QA_MATRIX_LOANREQUEST_V1_1.md) § Bloco 2. **A2b/A2c** permanecem **backlog** opcional. [`plans/completed/`](./plans/completed/) **só** histórico (ver [`plans/README.md`](./plans/README.md)).
...
| 2026-05-05 | **Mini ADR «Identidade pública e snapshots de nomes»** publicado em [`ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md`](./ADR_IDENTIDADE_PUBLICA_SNAPSHOTS_NOMES.md) — só documentação; sem `src/`, rules nem testes; implementação futura conforme §13 do ADR. |
