...
**ADR — Financeiro Local do Cliente / «Minhas dívidas»:** [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md) — modelo local **`clientDebtLedger`** separado de **`clients[]`** e de **`loanRequestConversionRegistry`**; dados **somente locais neste aparelho** (escopo); Firebase continua só vínculo/pedido **pré-financeiro** como contexto — **sem** sincronização financeira remota; **`calculations.js`**, **`firestore.rules`**, Firebase SDK e **`payment.linkContext`** **não** foram alterados nesta linha (**Subfases A–D1** entregues: **`5fc8a58`**, **`e24eb25`**, **`0f2c43b`**, **`40fa3a4`**); **D2** fecha registro documental da **D1**; ver ADR §13 para restante da Subfase D e Subfase E.
...
| 2026-05-16 | **«Minhas dívidas» — Subfases A–D1 no código + D2 documental:** **`5fc8a58`** (storage ledger) · **`e24eb25`** (resumo Fornecedores) · **`0f2c43b`** (detalhe fornecedor) · **`40fa3a4`** (backup/export/import/auto-backup incluem **`clientDebtLedger`**, compatível com arquivos antigos); **`calculations.js`**, **`firestore.rules`**, SDK Firebase e **`payment.linkContext`** intocados; docs vivos atualizados (**D2**). Próximo: smoke/encerramento ou restante Subfase D conforme ADR — **sem** Painel global (**Subfase E**) sem ADR própria. |
