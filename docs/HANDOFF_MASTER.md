...
- **Bloco 1 (execução parcial — `loanRequests`):** plano executável vivo em [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) — **Fase A1 concluída** (**`dcc9f80`**, **`4951bdf`**); **subfase A2a concluída** (decisões de arquivamento **só em docs**, **sem** código). **A2b/A2c** **não** implementadas. **Próxima subfase recomendada:** **`B1`** (métrica de saldo, análise — **sem** editar `calculations.js`) **salvo** decisão explícita de priorizar **A2b**. **B2, C–F** **não** concluídas aqui.
...
**Próximo recorte após esta entrega:** **Bloco 1** — **`B1`** (fluxo padrão após A2a) **ou** **`A2b`** se a governança priorizar implementação de arquivo antes do alerta de saldo; **`A2c` somente após `A2b`**. Ver [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md), [`CHECKPOINT_CHECKLIST.md`](./CHECKPOINT_CHECKLIST.md), [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md); [`plans/completed/`](./plans/completed/) **só** histórico (ver [`plans/README.md`](./plans/README.md)).
