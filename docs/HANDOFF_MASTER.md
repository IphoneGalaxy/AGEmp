...
- **Bloco 1 (`loanRequests` — operacional):** **funcionalmente fechado** (governança **Opção A**, 2026-05-04). Plano **arquivado** (histórico): [`plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./plans/completed/PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md). **Entregue:** **A1a** (`dcc9f80`, `countUnreadLoanRequests` + testes) · **A1b** (`4951bdf`, badges na Conta) · **A2a** (decisões arquivamento por lado, documental — **`8b3cadb`**) · **B1** (métrica **`availableMoney`** / **`calculateGlobalStats`**) · **B2** (`07ef7e5`, alerta não bloqueante no fornecedor). **A2b/A2c** **não** implementadas — **backlog / futuro** (organização por arquivamento); **não** impedem o fechamento funcional. **Próxima fase recomendada:** **Bloco 2 — Conversão Governada de LoanRequest aprovado em Contrato Local** — **fonte viva / ADR aprovado pela governança:** [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) · contexto em [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md). **Bloco2-0** (ADR/plano documental) **concluído** com **aprovação** registada; **Bloco2-A** é a **próxima subfase autorizada** — **código não iniciado**; **Bloco2-B–E** — **não concluídos**.
...
- **proxima fase recomendada (produto, 2026-05-04):** **Bloco 2** — conversão governada de pedido aprovado em contrato local — [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) (**aprovado** pela governança; ver roadmap [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md)); **implementação de código** — **Bloco2-A não iniciada** (primeira PR = entrada UX apenas); **D6** fechada no ADR (data do contrato = momento da conversão local no registo).
...
**Próximo recorte recomendado:** **Bloco 2 — Conversão Governada de LoanRequest aprovado em Contrato Local** — ADR/plano vivo [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) (**aprovado**); roadmap [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md). **Bloco2-0** documental **fechado com aprovação**; **Bloco2-A** **autorizada** como próximo incremento (**código não iniciado** — só detecção `approved` + botão «Registrar contrato local», sem criar contrato nem modal completo); **Bloco2-B–E** **não concluídos**. **A2b/A2c** permanecem **backlog** opcional. [`plans/completed/`](./plans/completed/) **só** histórico (ver [`plans/README.md`](./plans/README.md)).
...
| 2026-05-04 | **Governança Bloco 2:** ADR [`ADR_BLOCO2_CONVERSAO_GOVERNADA.md`](./ADR_BLOCO2_CONVERSAO_GOVERNADA.md) **aprovado** para orientar implementação; **Bloco2-A** **autorizada** (limites: sem criar contrato, sem modal completo, sem Firestore/`calculations.js`/rules nesta subfase); **D6** fechada (data contrato = conversão local); MVP **sem** marcação remota; **A2b/A2c** **backlog**; **código Bloco2-A** — **não iniciado**. |
