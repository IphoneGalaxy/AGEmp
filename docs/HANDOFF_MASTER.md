...
- **Bloco 1 (execução parcial — `loanRequests`):** plano executável vivo em [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) — **A1** concluída; **A2a** documental concluída (**`8b3cadb`**); **B1** (análise de métrica) **concluída** — **`availableMoney`** via **`calculateGlobalStats`** para a futura **B2** (`requestedAmount/100` vs reais). **A2b/A2c** e **B2** **não** implementadas. **Próxima recomendada:** **`B2`** **salvo** priorizar **A2b**. **C–F** fora deste registro.
...
**Bloco 1 — Subfase B1 (métrica para alerta B2 — análise concluída):** confirmado uso de **`availableMoney`** (retorno de **`calculateGlobalStats`** em `calculations.js`), alinhado ao **“Total Disponível”** do Painel. **B2** deverá comparar **`requestedAmount / 100`** (reais) com **`availableMoney`**; alerta **informativo**, **não bloqueante**, **referência local** (não saldo bancário); **sem** alterar o motor; **sem** escrita Firestore motivada pelo alerta. Ver [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) §6 B1.

**Próximo recorte após esta entrega:** **Bloco 1** — **`B2`** (implementação do alerta no painel fornecedor, com encadeamento de props desde `App`/`Settings`/`AccountScreen` se necessário) **ou** **`A2b`** se a governança priorizar arquivamento antes; **`A2c` somente após `A2b`**. Ver [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md), [`CHECKPOINT_CHECKLIST.md`](./CHECKPOINT_CHECKLIST.md), [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md); [`plans/completed/`](./plans/completed/) **só** histórico (ver [`plans/README.md`](./plans/README.md)).
