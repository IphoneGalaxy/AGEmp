...
- **Bloco 1 (execução parcial — `loanRequests`):** plano executável vivo em [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) — **A1** concluída; **A2a** documental concluída (**`8b3cadb`**); **B1** (análise de métrica) **concluída** — **`availableMoney`** via **`calculateGlobalStats`**. **B2** **concluída** — **`07ef7e5`**: alerta informativo e **não bloqueante** no **`LoanRequestsSupplierPanel`**, repasse **`availableMoney`** (**`App.jsx` → `Settings.jsx` → `AccountScreen.jsx`**), comparação **`requestedAmount / 100 > availableMoney`**, estados **`pending`** / **`under_review`** apenas; **sem** alterar **`calculations.js`**, **`firestore.rules`**, schema, transições, gravação Firestore pelo alerta, **`payment.linkContext`**, sync financeiro remoto ou contrato automático. **A2b/A2c** **não** implementadas; **fechamento integral** do Bloco 1 (ex.: mover plano a **`docs/plans/completed/`**) **pendente** de governança. **Bloco 2** (conversão governada pedido → contrato local) registado em [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md) — **só** planejamento. **Próxima subfase típica:** **A2b** **ou** fechamento documental amplo **salvo** decisão explícita. **C–F** fora deste registro implementativo.
...
**Bloco 1 — Subfase A2a (decisões de arquivamento, documental — concluída):** contrato **por lado** (`archivedByClient` / `archivedBySupplier` **planejados**, **não** no Firestore ainda); **só** status terminais; desarquivar **permitido**; **`updatedAt` intocado** no arquivo/desarquivo; exclusão/`delete` **fora**; rules/UI **só** nas futuras **A2b**/**A2c** — detalhe em [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) §6.

**Bloco 1 — Subfase B1 (métrica para alerta B2 — análise concluída):** confirmado uso de **`availableMoney`** (retorno de **`calculateGlobalStats`** em `calculations.js`), alinhado ao **“Total Disponível”** do Painel. **B2** compara **`requestedAmount / 100`** (reais) com **`availableMoney`**; alerta **informativo**, **não bloqueante**, **referência local** (não saldo bancário); **sem** alterar o motor; **sem** escrita Firestore motivada pelo alerta. Ver [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md) §6 B1.

**Bloco 1 — Subfase B2 (alerta fornecedor — concluída):** **`07ef7e5`** — repasse **`availableMoney`** desde **`globalStats`**: **`App.jsx`**, **`Settings.jsx`**, **`AccountScreen.jsx`**, **`LoanRequestsSupplierPanel.jsx`**. Alerta só em **`pending`** / **`under_review`** quando **`requestedAmount / 100 > availableMoney`**; **sem** terminais nem **`counteroffer`** no escopo validado; aprovação **não** bloqueada; **`calculations.js`**, **`firestore.rules`**, schema, transições, **`payment.linkContext`**, sync remoto e contrato automático **intocados**. Ver plano executável §6 B2 e [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md) (Fase B).

**Próximo recorte após esta entrega:** **Bloco 1** — **`A2b`** (campos + rules de arquivamento) **ou** **fechamento documental** do bloco neste plano (incl. eventual mover arquivo a **`docs/plans/completed/`**) conforme governança; **`A2c` somente após `A2b`**. **Bloco 2** — conversão governada de pedido aprovado em contrato local — **só** em [`LOANREQUEST_EVOLUTION_ROADMAP.md`](./LOANREQUEST_EVOLUTION_ROADMAP.md), **ADR/planejamento obrigatório antes de código**. Ver [`PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md`](./PLANEJAMENTO_BLOCO1_LOANREQUEST_OPERACIONAL.md), [`CHECKPOINT_CHECKLIST.md`](./CHECKPOINT_CHECKLIST.md); [`plans/completed/`](./plans/completed/) **só** histórico (ver [`plans/README.md`](./plans/README.md)).
...
| 2026-05-04 | **Subfase B2 concluída:** **`07ef7e5`** — alerta não bloqueante no painel fornecedor; roadmap atualizado com **Bloco 2** (conversão governada) como fase futura **sem** código. **Fechamento integral** do Bloco 1 (mover plano a `completed/`) adiado à próxima rodada. |
