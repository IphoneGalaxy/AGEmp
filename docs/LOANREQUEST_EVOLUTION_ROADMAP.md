...
- **Financeiro local do Cliente («Minhas dívidas»):** **planejamento documentado** — [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md). É **ortogonal** às fases **A1–F** deste roadmap: **`loanRequests`** permanece só pré-financeiro; o ledger do cliente (**`clientDebtLedger`**) não torna Firebase fonte de saldo nem espelha o contrato local do fornecedor; **implementação não iniciada** por este registro.
