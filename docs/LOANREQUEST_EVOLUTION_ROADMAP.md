...
- **«Visão Fornecedores + Governança de vínculo/local»:** **FECHADO** (Pacotes **1–3**) — [`ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md`](./ADR_VISAO_FORNECEDORES_GOVERNANCA_VINCULO_LOCAL.md) §16 · **`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`**. **Sem** mudança Firestore/`loanRequests`/`firestore.rules`; UX/registry/arquivamento **`archivedAt`** **locais**.
- **Financeiro local do Cliente («Minhas dívidas»):** **funcionalmente fechado nesta etapa** — [`ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md`](./ADR_FINANCEIRO_LOCAL_CLIENTE_MINHAS_DIVIDAS.md). É **ortogonal** às fases **A1–F** deste roadmap: **`loanRequests`** permanece só pré-financeiro; **`clientDebtLedger`** é **dados locais neste aparelho**, **sem sincronização financeira remota**, **sem** Firebase como fonte autoritativa do passivo local; Subfases **A–D3** (**`5fc8a58`** … **`eedbd2e`**, smoke **D3** OK **2026-05-17**) — ver ADR §13–§20. **Pacote «Pré-Sync Local Hardening»** (UX fronteira plataforma/local, lembretes derivados, export JSON específico — commits **`d6f69af`**, **`8228b5c`**, **`b86ae9b`**) **fechado documentalmente** **2026-05-16** — [`plans/completed/PRE_SYNC_LOCAL_HARDENING.md`](./plans/completed/PRE_SYNC_LOCAL_HARDENING.md); smoke ADR §21 / matriz QA; **sem** iniciar sync financeiro remoto neste pacote.
...
**Bloco 1** encerrado funcionalmente — ver plano arquivado. **Bloco 2 (Fase F / conversão governada)** — **fechado**. **Mini ADR snapshots de nomes** — **fechada** (**`6793461`** … **`28f3f4a`**). **«Visão Fornecedores + Governança local»** — **fechada** (**`0be3e0b`**, **`c921d8d`**, **`a6c2d8c`** — §16 da ADR). Ordem sugerida para o que segue: **A2b/A2c** quando priorizado (arquivamento **remoto** por lado — distinto do arquivamento **local** já entregue); **C** após avaliação da Fase **B** em uso; **D** / **E** conforme decisão e ADR.
