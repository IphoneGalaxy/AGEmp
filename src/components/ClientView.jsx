 ...
 *
 * Tela completa (overlay) que exibe:
 * - Resumo de dívida do cliente
 * - Formulário para novo empréstimo
 * - Lista de contratos (empréstimos) com edição/exclusão
 * - Pagamentos por contrato com edição/exclusão
 * - Geração de extrato para copiar
 *
 * @param {Object} props
 * @param {Object}   props.clientData - Dados processados do cliente (do motor de cálculos).
 * @param {number}   props.availableMoney - Saldo disponível em caixa.
 * @param {Function} props.onUpdateClients - Callback que recebe uma função updater para atualizar clientes.
 * @param {Function} props.onClose - Callback para fechar a visão do cliente.
 * @param {Function} props.showToast - Callback para exibir notificação toast.
 */
const ClientView = ({ clientData, availableMoney, onUpdateClients, onClose, showToast }) => {
      ...
      clients.map((c) => {
        if (c.id === clientData.id) {
          return {
            ...c,
            loans: [
              { id: generateId(), date: newLoanDate, amount: amountToLend, payments: [] },
              ...c.loans,
            ],
          };
        }
      ...
      clients.map((c) => {
        if (c.id === clientData.id) {
          return {
            ...c,
            loans: c.loans.map((l) =>
              l.id === editingLoan.id ? { ...l, date: editingLoan.date, amount: newAmount } : l
            ),
          };
        }
        return c;
  ...
  const generateStatement = () => {
    ...
    clientData.loans.forEach((loan) => {
      text += `📌 *Empréstimo: ${formatMoney(loan.amount)} (${formatDate(loan.date)})*\n`;
      if (loan.isPaidOff) {
  ...
  return (
    <div className="p-4 flex flex-col h-screen bg-gray-50 absolute top-0 left-0 w-full z-10 overflow-y-auto pb-24">
        ...
        >
          <div className="flex justify-between items-center mb-3">
            ...
            <span className="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded border border-blue-100 font-medium">
              Caixa: {formatMoney(availableMoney)}
            </span>
          </div>
          <input
            type="date"
          ...
          <input
            ...
            placeholder="Valor (R$)"
            required
            className="w-full mb-4 p-3 border rounded-xl bg-gray-50"
          />
          <div className="flex gap-2">
            <button
              type="button"
      ...
      {/* Lista de contratos (empréstimos) */}
      <div className="space-y-4">
        {clientData.loans.map((loan) => (
          ...
          >
            {/* Cabeçalho do contrato: edição / exclusão / normal */}
              ...
              <div className="p-4 bg-gray-50 border-b border-gray-100 flex justify-between items-center relative">
                <div>
                  ...
                  <p className="font-black text-gray-800 text-lg">
                    Empréstimo: {formatMoney(loan.amount)}
                  </p>
                </div>
                ...
                <div className="flex gap-2 text-gray-400 mt-6">
                  <button
                    onClick={() =>
                      setEditingLoan({ id: loan.id, date: loan.date, amount: loan.amount })
                    }
                ...
                <>
                  ...
                  <div className="flex justify-between items-center bg-orange-50 px-3 py-2 rounded-lg border border-orange-100 mb-4">
                    <p className="text-xs text-orange-800 font-medium">
                      Juros (10%): {formatMoney(loan.baseInterest)}
                    </p>
                    <p className="text-sm text-orange-900 font-black">
                      Quitação: {formatMoney(loan.currentPrincipal + loan.baseInterest)}
                    </p>
                  </div>
                          ...
                          <div className="flex justify-between items-center">
                            <div>
                              ...
                              <p className="text-[10px] text-gray-500 ml-3 mt-0.5">
                                Juros: {formatMoney(p.interestPaid)} | Abateu:{' '}
                                {formatMoney(p.amortized)}
                              </p>
                            </div>
                            <div className="flex gap-2 text-gray-400">
                              ...
                              <button
                                onClick={() =>
                                  setConfirmDeletePaymentId({ loanId: loan.id, id: p.id })
                                }
